...
本アプリでは「割当先（予定の札）」と「割当済（実際に札が付いた）」を別々に管理しているため、`area` だけでは往復（生成→取り込み）で情報が失われます。そのため生成時に以下のプロパティを追加しています。

| キー           | 型        | 説明                                                                                        |
| :------------- | :-------- | :------------------------------------------------------------------------------------------ |
| `ktm_assigned` | `boolean` | 「割当済」かどうか。`area` は割当先の札IDを常に出力し、割当済かどうかはこちらで表現する。 |
| `ktm_stage`    | `string`  | 割当先の海域（例: `"E-4-3"`）。札名は含めず海域のみ。割当先が未設定の場合は出力しない。 |

ゲーム／他ツール由来のコードにはこれらのプロパティが存在しないため、取り込み時は以下のように補完します。他ツールは未知のプロパティを無視するため、互換性には影響しません。

- `ktm_assigned` が無ければ `area > 0` を割当済みとみなす。
- `ktm_stage` が無ければ `area` の札が使われている最初の海域を割当先とみなす。同じ札が複数の海域で使われる場合、正しい海域は特定できない（そのために `ktm_stage` を出力している）。取り込み時は `ktm_stage` の海域でその札が使えるかを検証し、使えなければ札からの推測にフォールバックする。
