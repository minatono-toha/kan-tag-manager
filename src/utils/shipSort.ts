...
// 改装段階の選択UI(▼ポップアップ・詳細モーダル)で、別 spGroupId に分割されている艦
// (=一覧に出てこない/選べない改装段階)を案内する文言。艦種以外(国籍等)が変わって
// 特攻倍率が割れるケースもあるため、原因を「艦種」に限定しない一律の文言にしている。
export const SP_GROUP_SPLIT_NOTE = '特攻倍率が変わる艦(改装後に艦種や国籍などが変わる場合)は別の行で扱っています'
