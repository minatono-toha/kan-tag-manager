...
- ローカル保存
  入力データ(所持艦娘・札の割り当てなど)はすべてブラウザのIndexedDBに保存され、外部サーバーへ送信されることはありません。なお、サイトの表示に必要なマスタデータ(イベント情報・艦船一覧など)の取得のためFirebase(Firestore)との通信、および利用状況把握のためGoogle Analyticsによるアクセス解析を行っています。
